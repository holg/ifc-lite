...
pub fn extrude_profile(
    ...
    if depth <= 0.0 {
        return Err(Error::geometry("Depth must be positive"));
    }
...
pub fn extrude_profile_with_voids(
    ...
    let side_vertex_count = profile_with_holes.outer.len() * 2
        + profile_with_holes
            ...
            .map(|h| h.len() * 2)
            .sum::<usize>();
    let partial_void_vertices = partial_void_count * 100;
    ...
    for void_info in profile_with_voids.partial_voids() {
        create_partial_void_geometry(void_info, depth, &mut mesh)?;
    }
...
fn create_partial_void_geometry(void_info: &VoidInfo, total_depth: f64, mesh: &mut Mesh) -> Result<()> {
    ...
    let void_profile = Profile2D::new(void_info.contour.clone());
    ...
    if void_info.depth_start > epsilon {
        create_cap_mesh(
            &void_triangulation,
    ...
    if void_info.depth_end < total_depth - epsilon {
        create_cap_mesh(
            &void_triangulation,
            void_info.depth_end,
            Vector3::new(0.0, 0.0, 1.0),
